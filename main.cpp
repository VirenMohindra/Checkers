//  Copyright (c) 2014 Viren Mohindra. All rights reserved.
...
int main()
{
    ...
    cout << "|     |  |  |   __|     |  |  |   __| __  |   __| \n";
    cout << "|   --|     |   __|   --|    -|   __|    -|__   | \n";
    cout << "|_____|__|__|_____|_____|__|__|_____|__|__|_____| \n";
...
bool check_move() //constructs restrictions based on rules of checkers and handles errors associated with it.
{
    ...
    if (board[row2][column2] != ' '){leap = false; return false;}
    
    //checks if location entered by the user contains a piece to be moved.
    if (board[row1][column1] == ' '){leap = false; return false;}
    
    //checks if the piece isn't moving diagonally.
    if (column1 == column2 || row1 == row2){leap = false; return false;}
    
    //checks if the piece is moving by more than 1 column and only 1 row
    if ((column2 > column1 + 1 || column2 < column1 - 1) && (row2 == row1 +1 || row2 == row1 - 1)) {leap = false; return false;}
    ...
    if (row2 > row1 + 1 || row2 < row1 - 1)
    {
        ...
        if (column2 != column1 + 2 && column2 != column1 - 2){leap = false; return false;}
        
        //checks if the piece is leaping over another piece.
        if (row2 > row1 && column2 > column1)
        {if (board[row2 - 1][column2 - 1] == ' '){leap = false; return false;}}
...
{
    {
        ...
        // open the highscore file in write mode.
        ofstream outfile;
        ...
        cout << "Reading from the highscore file" << endl;
        infile >> data;
        cout << "\nName: " << data << endl;
        infile >> data;
        cout << "\nAge: " << data << endl;
        infile >> data;
        cout<< "\nTime game was won:" << Hour << ":" << Min << ":" << Sec;
        infile.close();
    }
}
